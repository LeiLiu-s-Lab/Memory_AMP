%% CR-GD-MAMP (overflow-avoiding)
% ====================================================================
function [MSE, Var, x_post] = MAMP(index_ev, x, y, dia, v_n, L, it, info)
    M = length(y);
    N = length(x);
    delta = M / N;
    ...
    z = zeros(M, it);
    z(:, 1) = y - A_times_x(x_phi(:, 1), index_ev, dia, M, N);
    ...
    for t = 1 : it
        % MLE
        [log_vth, sgn_xi, z_hat, r_hat, r, v_gam] = MLE_MAMP(index_ev, dia, x_phi, v_phi, chi, ...
            sgn_xi, log_vth, y, z_hat, r_hat, w_0, wb_00, lam_s, v_n, log_th1, t);
        v_gam = real(v_gam);
        % NLE
        [x_post, v_post] = Demodulator(r, v_gam, info);
        res_list(:, t) = x_post;
        ...
        if Var(t) <= thres_0
            ...
            break
        elseif t > 2
            % for robustness in some special cases 
            % okay for deleting this part
        ...
        z(:, t+1) = y - A_times_x(x_phi(:, t+1), index_ev, dia, M, N); 
...
%% Only for robustness in some special cases
